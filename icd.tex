...
\begin{description}
    ...
    \item[cachedir (string)] \hfill \\ Path to cache directory
    \item[nEZ (list)] \hfill \\ a list of number of exo-zodi % This does not exist in ZL now, or if it does, this is not the name anymore **NEW**
    \item[commonSystemfEZ (boolean)] \hfill \\ A boolean indicating whether each star has a common number of Exozodi % There's also a line missing for commonSystemfEZ **NEW**
    \item[fZminglobal (astropy Quantity)] \hfill \\ Default global possible local zodical light minimum ignoring any constraints in units of $ 1/arcsec^2 $ % **NEW**
\end{description}
\end{itemize}
...
\begin{description}
    ...
    \item[absTimefZmin (astropy Time array)] \hfill \\ absolute time in MJD that minimum zodiacal light will occur next
    \item[cachedir (string)] \hfill \\ Path to cache directory
    \item[fZMap (dict)] \hfill \\ Contains the fZ values (in units of $1/arcsec^2$) for each star in the Target List over an entire orbit of the sun for each system mode % **NEW**
    \item[fZQuads (dict)] \hfill \\ Contains the list of zodiacal light minimums (in units of $1/arcsec^2$) for each star and the times (in units of days in MJD) they occur at for each system mode % **NEW**
    \item[integrationFilterTimes (astropy Quantity array)] \hfill \\ integration times of the stars (in absolute time in MJD) % **NEW** ******CHANGE THE NAME OF THIS IN EXOSIMS. THE GREP IS SET UP WITH THE LIST OF FILES*******
    \item[integrationFilterInds (integer ndarray)] \hfill \\ indices of the viable stars after filtering based on integration time % **NEW**
\end{description}
\end{itemize}
